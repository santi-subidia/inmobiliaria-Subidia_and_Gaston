...
namespace Inmobiliaria.Repositories
{
    public class PagoRepository : IPagoRepository
    {
        ...
        // Lecturas
        // =========================
        public async Task<(IEnumerable<Pago> items, int total)> GetPagedAsync(int page, int pageSize, int? contratoId = null)
        {
            if (page <= 0) page = 1;
        ...
        public async Task<long> CreateAsync(Pago p)
        {
            ...
            cmd.Parameters.AddWithValue("@creado_por", p.CreadoPor ?? "sistema");
            cmd.Parameters.AddWithValue("@creado_at", p.CreadoAt == default ? DateTime.UtcNow : p.CreadoAt);
            cmd.Parameters.AddWithValue("@anulado_por", (object?)p.AnuladoPor ?? DBNull.Value);
        ...
        public async Task<bool> ExistsByContratoAndNumeroAsync(int contratoId, int numeroPago, long? excludeId = null)
        {
            using var conn = _connectionFactory.CreateConnection();
            await conn.OpenAsync();
        ...
        private static Pago MapPago(MySqlDataReader r) => new()
        {
            ...
            CreadoPor    = r.GetString("creado_por"),
            CreadoAt     = r.GetDateTime("creado_at"),
            AnuladoPor   = r.IsDBNull(r.GetOrdinal("anulado_por")) ? null : r.GetString("anulado_por"),
